...
class RoutingStackInstructionSyntaxTest {
    ...
    fun popUntil() {
        val stack =
            RoutingStack.from(RouteImpl(0), RouteImpl(1), RouteImpl(2), RouteImpl(1), RouteImpl(2))
    ...
    fun replaceTopWith_onNonEmptyStack_withDuplicateKey() {
        val stack = RoutingStack.from(RouteImpl(0), RouteImpl(1), RouteImpl(2))
        val newStack =
            stack.replaceTopWith(RoutingStack.Element(RouteImpl(3), key = stack.first().key))
        assertEquals(2, newStack.elements.size)
        assertEquals(RouteImpl(1), newStack.elements[0].route)
